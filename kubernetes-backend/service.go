...
// watchServices monitors the kubernetes health checks and creates a new configuration
// on every change.
func watchServices(client *http.Client, config chan string) {
	...
	for {
		...
		if err != nil {
			log.Printf("[WARN] kubernetes: Error fetching health state. %v", err)
			time.Sleep(time.Second)
			continue
		}

		log.Printf("[DEBUG] kubernetes: Health changed to #%d", meta.LastIndex)
...
func servicesConfig(client *api.Client, checks []*api.HealthCheck, tagPrefix string) string {
	...
	for _, check := range checks {
		// Make the node part of the id, because according to the Kubernetes docs
		// the ServiceID is unique per agent but not cluster wide
		// https://www.kubernetes.io/api/agent/service.html#id
...
func serviceConfig(client *api.Client, name string, passing map[string]bool, tagPrefix string) (config []string) {
	...
	if err != nil {
		log.Printf("[WARN] kubernetes: Error getting datacenter. %s", err)
		return nil
	}
	...
	if err != nil {
		log.Printf("[WARN] kubernetes: Error getting catalog service %s. %v", name, err)
		return nil
	}
	...
	for _, svc := range svcs {
		...
		for _, tag := range svc.ServiceTags {
			if route, opts, ok := parseURLPrefixTag(tag, tagPrefix, env); ok {
				...
				// use kubernetes node address if service address is not set
				if addr == "" {
					addr = svc.Address
				}
